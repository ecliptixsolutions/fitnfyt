...
const colorFromName = (name) => {
  const colors = ["bg-emerald-600", "bg-sky-600", "bg-amber-600", "bg-rose-600", "bg-violet-600", "bg-cyan-600"];
  let h = 0;
...
function AppShell({
  ...
  const memberResults = normalizedSearch ? members.filter(
    (member) => `${member.name} ${member.phone} ${member.email ?? ""} ${member.plan}`.toLowerCase().includes(normalizedSearch)
  ).slice(0, 4) : [];
  const leadResults = normalizedSearch ? leads.filter(
    (lead) => `${lead.name} ${lead.phone} ${lead.enquiry}`.toLowerCase().includes(normalizedSearch)
  ).slice(0, 2) : [];
  const planResults = normalizedSearch ? [...new Set(members.map((member) => member.plan))].filter((plan) => plan.toLowerCase().includes(normalizedSearch)).slice(0, 2) : [];
  const notificationCount = members.filter((member) => member.status === "expiring").length + payments.filter((payment) => payment.status === "Pending").length + leads.filter((lead) => lead.status === "New").length;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-h-screen bg-background text-foreground", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `transition-[padding] ${collapsed ? "lg:pl-20" : "lg:pl-64"}`, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("header", { className: "sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex h-16 items-center gap-3 px-4 lg:px-6", children: [
